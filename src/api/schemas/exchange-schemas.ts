    ...
    orders: {
      ...
      items: {
        ...
        properties: {
          ...
          createdAt: { type: 'string', format: 'date-time', description: 'ISO timestamp when the order was created' },
          closedAt: { type: 'string', format: 'date-time', description: 'ISO timestamp when the order was closed' },
          exchange: { type: 'string', description: 'Exchange name (kraken, mexc, etc.)' },
        },
        required: ['orderId', 'pair', 'direction', 'type', 'status', 'amount', 'amountExecuted', 'price', 'limitPrice', 'cost', 'createdAt', 'closedAt', 'exchange'],
        additionalProperties: false,
      },
    },
    timestamp: { type: 'string', format: 'date-time' },
