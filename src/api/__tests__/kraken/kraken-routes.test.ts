...
describe('Kraken Exchange Routes', () => {
  ...
  describe('GET /api/v1/kraken/orders/closed', () => {
    ...
    it('should handle empty closed orders response', async () => {
      ...
      expect(Object.keys(body.orders.closed || {})).toEqual([]);
      expect(body.timestamp).toBeDefined();
    });
  });
  ...
  describe('DELETE /api/v1/kraken/orders/cancel/:txid', () => {
    it('should cancel an order successfully', async () => {
      ...
      expect(response.statusCode).toBe(204); // Test mode returns 204 for safety
      // In test mode, cancel operations return 204 with no body for safety
      expect(response.body).toBe('');

      // In test mode, the cancel operation is blocked for safety, so no API call is made
